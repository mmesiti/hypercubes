...
def get_indices_tree(partitioning, coordinates):
    ...
    def iterate_children(node):
        ...
        return [(sub_partitionings[child_type],
                 rests,
        ...
        if partitioning:
            ...
            return [
                (
                    i.idx,  #
                    i.cached_flag,  #
                    sub_partitionings[i.child_type] if sub_partitionings else None,  #
                    i.rests,  #
                    partitioning.name)  #
...
def get_relevant_indices_flat(tree_indices):
    ...
    return [(len([_ for _, flag, name in index if flag]), # count of cache flags
