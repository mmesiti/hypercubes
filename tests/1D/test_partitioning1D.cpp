...
BOOST_FIXTURE_TEST_CASE(test_sizeparity_info_list, Q1DFixture) {
  ...
  std::vector<PartInfo> exp_sp{
      {9, Parity::ODD}, {11, Parity::EVEN}, {11, Parity::ODD}};
  auto res = partitioning1D.sub_partinfo_kinds();
  BOOST_TEST(exp_sp == res);
}

namespace bdata = boost::unit_test::data;
BOOST_DATA_TEST_CASE_F(Q1DFixture, test_idx_to_partinfo_kind, bdata::xrange(4),
                       i) {
  int child_kind;
  if (i == 0 or i == 2)
    child_kind = 1;
  ...
  BOOST_TEST(child_kind == partitioning1D.idx_to_partinfo_kind(i));
}
...
BOOST_FIXTURE_TEST_CASE(test_idx_to_partinfo_kind_throws, Q1DFixture) {
  BOOST_CHECK_THROW(partitioning1D.idx_to_partinfo_kind(55),
