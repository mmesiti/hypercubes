...
BOOST_AUTO_TEST_CASE(test_eo_naive_1d_even) {
  ...
  auto t_partitioned = eo_naive(t, 0);
...
BOOST_AUTO_TEST_CASE(test_eo_naive_1d_even_nested) {
  auto t = generate_nd_tree({4});
  auto t_nested = q(t, 0, 1); // Adding a level
  ...
  auto t_expected = mtkv(false, {{{}, // key added by q is empty
                                  t_sub_expected}});
  BOOST_TEST(*t_partitioned == *t_expected);
}
BOOST_AUTO_TEST_CASE(test_eo_naive_1d_odd_nested) {
  auto t = generate_nd_tree({5});
  auto t_nested = q(t, 0, 1); // Adding a level
  auto t_sub_expected = mtkv(false, {{{0},
                                      mtkv(false, {{{0}, mtkv(true, {})},    //
...
BOOST_AUTO_TEST_CASE(test_index_pullback_id) {
  auto t = generate_nd_tree({2, 2, 2});
  vector<int> in{0, 1, 0};
  auto out = index_pullback(t, in, 0, 3);
  BOOST_TEST(in == out);
}
...
BOOST_AUTO_TEST_CASE(test_index_pullback_complex) {
  ...
  auto out = index_pullback(t0, in, 0, 3);
...
BOOST_AUTO_TEST_CASE(test_index_pushforward_id) {
  auto t = generate_nd_tree({2, 2, 2});
  vector<int> in{0, 1, 0};
  auto out = index_pushforward(t, in, 0, 3);
...
BOOST_AUTO_TEST_CASE(test_index_pushforward_complex) {
  ...
  auto out = index_pushforward(t0, in, 0, 3)[0];
  BOOST_CHECK_EQUAL_COLLECTIONS(out.begin(), out.end(), //
                                out_expected.begin(), out_expected.end());
}
...
BOOST_AUTO_TEST_CASE(test_index_pullback_and_pushforward) {
  ...
  {
    vector<int> in{0, 2, 0};
    auto pullback = index_pullback(t0, in, 0, 3);
    auto pushforward = index_pushforward(t0, pullback, 0, 3)[0];
  ...
  /// In both ways!
  {
    ...
    auto pushforward = index_pushforward(t0, in, 0, 3)[0];
    auto pullback = index_pullback(t0, pushforward, 0, 3);
    BOOST_CHECK_EQUAL_COLLECTIONS(in.begin(), in.end(), //
...
BOOST_AUTO_TEST_CASE(test_index_pullback_throws) {
  ...
  BOOST_CHECK_THROW(index_pullback(t, {1, 4, 3}, 0, 3), KeyNotFoundError);
}
...
BOOST_AUTO_TEST_CASE(test_index_pushforward_noresults) {
  auto t = generate_nd_tree({2, 4, 4});
  auto res = index_pushforward(t, {1, 4, 3}, 0, 3);
...
BOOST_AUTO_TEST_CASE(test_index_pullback_q) {
  ...
  auto out =
      index_pullback(t1, in, 0, in.size()); // because q creates 2 new levels
...
BOOST_AUTO_TEST_CASE(test_index_pushforward_q) {
  ...
  auto out = index_pushforward(t1, in, level, level + 2)[0];
  BOOST_CHECK_EQUAL_COLLECTIONS(out.begin(), out.end(), //
                                out_exp.begin(), out_exp.end());
}
...
BOOST_AUTO_TEST_CASE(test_index_pullback_inverse_tree) {
  ...
  int end_level = level + 2;
  auto t2 = flatten(t1, start_level, end_level); // the inverse tree

  vector<int> in2{0, 3, 2};
  vector<int> out2_exp{0, 1, 1, 2};
  vector<int> out1_exp{0, 3, 2}; // = in2;

  auto out2 = index_pullback(t2,                          //
                             in2,                         //
                             start_level,                 //
                             start_level + 1);            //
  auto out1 = index_pullback(t1,                          //
                             out2,                        //
                             level,                       //
                             level + 2);                  //
...
BOOST_AUTO_TEST_CASE(test_index_pushforward_both_ways) {
  ...
  auto out2 = index_pushforward(t1,                       //
                                in2,                      //
                                level,                    //
                                level + 2)[0];            //
  auto out1 = index_pushforward(t2,                       //
                                out2,                     //
                                start_level,              //
                                end_level)[0];            //
  BOOST_CHECK_EQUAL_COLLECTIONS(out2.begin(), out2.end(), //
...
BOOST_AUTO_TEST_CASE(test_remap_level_pullback_index) {
  ...
  vector<int> in{0, 3, 0};
  auto out = index_pullback(t1, in, level, level + 1);
  vector<int> out_expected{0, 11, 0};
  BOOST_CHECK_EQUAL_COLLECTIONS(out.begin(), out.end(), //
                                out_expected.begin(), out_expected.end());
}
BOOST_AUTO_TEST_CASE(test_remap_level_pushforward_index_multiple) {
  ...
  vector<int> in{0, 12, 8};
  auto outs = index_pushforward(t1, in, level, level + 1);
  vector<vector<int>> outs_expected{{0, 1, 8}, {0, 3, 8}};
...
BOOST_AUTO_TEST_CASE(test_flatten_pullback) {
  ...
  int level_start = 0;
  int level_end = 2;
  auto t_collapsed = flatten(t, level_start, level_end);
  ...
  auto out = index_pullback(t_collapsed,      //
                            {2, 0},           //
                            level_start,      //
                            level_start + 1); //
...
BOOST_AUTO_TEST_CASE(test_flatten_pushforward) {
  ...
  auto t_collapsed_expected = mtkv(false, {{{0, 0}, subtree}, //
                                           ...
                                           {{1, 0}, subtree}, //
                                           {{1, 1}, subtree}});
  auto out = index_pushforward(t_collapsed,   //
                               {1, 0, 0},     //
                               level_start,   //
                               level_end)[0]; //
...
BOOST_AUTO_TEST_CASE(test_pullback_q_after_doubleremap) {
  // This test should fail if some of the ranges in pullback
  // are set incorrectly.
  auto t = generate_nd_tree({3, 4, 4});
  auto t05 = remap_level(t, 0, {1, 2, 0});     // remapping before the range
  auto t1 = remap_level(t05, 2, {2, 3, 0, 1}); // remapping after before
  int level = 1;
  auto t2 = q(t1, level, 2);
  auto pullback2 = index_pullback(t2, {2, 1, 0, 3}, level, level + 2);
  decltype(pullback2) exp_pullback2{2, 2, 3};
  BOOST_CHECK_EQUAL_COLLECTIONS(pullback2.begin(), pullback2.end(), //
                                exp_pullback2.begin(), exp_pullback2.end());
}

BOOST_AUTO_TEST_CASE(test_pushback_q_before_doubleremap) {
  // This test should fail if some of the ranges in pullback
  // are set incorrectly.
  auto t = generate_nd_tree({3, 4, 4});
  auto t05 = remap_level(t, 0, {1, 2, 0});     // remapping before the range
  auto t1 = remap_level(t05, 2, {2, 3, 0, 1}); // remapping after the range
  int level = 1;
  auto t2 = q(t1, level, 2);
  auto pushforward2 = index_pushforward(t2, {2, 2, 3}, level, level + 2)[0];
  decltype(pushforward2) exp_pushforward2{2, 1, 0, 3};
  BOOST_CHECK_EQUAL_COLLECTIONS(pushforward2.begin(), pushforward2.end(), //
                                exp_pushforward2.begin(),
                                exp_pushforward2.end());
}
